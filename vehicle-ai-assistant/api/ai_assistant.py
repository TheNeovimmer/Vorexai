...
class VehicleAIAssistant:
    ...
    def _generate_mock_response(self, user_message):
        ...
        if "insurance" in user_message.lower():
            ...
            return (
                f"Your last maintenance was on {last_maintenance} and the next scheduled maintenance is on {next_maintenance}. Recent maintenance includes: "
                + ", ".join(
                    [
                        f"{record['type']} on {record['date']}"
                        for record in vehicle["maintenanceRecords"][:3]
                    ]
                )
            )
            ...
            current_issues = [
                issue for issue in vehicle["issues"] if issue["status"] != "RESOLVED"
            ]
            if current_issues:
